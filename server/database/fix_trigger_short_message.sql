...
-- ======================================================
-- 1. FIX TRIGGER VALIDASI DUPLIKAT PENGADUAN (SHORT MSG)
-- ======================================================
...
BEGIN
  ...
  SELECT 
      COUNT(*) AS total,
      LEFT(GROUP_CONCAT(DISTINCT status SEPARATOR ', '), 30) -- batasi biar aman
  ...
  IF report_count >= 1 THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = CONCAT('Sudah ada laporan aktif: ', existing_status);
  END IF;
...
-- ======================================================
-- 2. TAMBAH KOLOM GAMBAR BUKTI SELESAI (SAFE ALTER)
-- ======================================================

-- Tambah kolom gambar URL
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS
               WHERE table_name = 'pengaduan_sarpras_pengaduan'
               AND column_name = 'gambar_bukti_selesai') THEN
  ALTER TABLE pengaduan_sarpras_pengaduan
    ADD COLUMN gambar_bukti_selesai TEXT NULL
    COMMENT 'URL bukti penyelesaian';
END IF;

-- Tambah kolom file_id
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS
               WHERE table_name = 'pengaduan_sarpras_pengaduan'
               AND column_name = 'file_id_bukti_selesai') THEN
  ALTER TABLE pengaduan_sarpras_pengaduan
    ADD COLUMN file_id_bukti_selesai VARCHAR(255) NULL
    COMMENT 'File ID bukti penyelesaian';
END IF;
